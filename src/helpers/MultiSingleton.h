...
public:
  ...
  static void setDefaultInstance(std::string key) { m_defaultKey = key; }
  ...
  bool isPropagator() { return m_isPropagator; }

  void setIsPropagator(bool value) { m_isPropagator = value; }
...
template<typename T>
std::map<std::string, T *> MultiSingleton<T>::m_instances;
template<typename T>
...
template<typename T>
bool MultiSingleton<T>::m_isPropagating = false;
