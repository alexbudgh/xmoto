  ...
  if (XMSession::instance()->webConfAtInit()) {
    ...
    StateMessageBox *v_msgboxState =
      new StateMessageBox(this,
                          std::string(GAMETEXT_ALLOWINTERNETCONN),
...
void StateEditWebConfig::checkEvents() {
  /* Get some pointers */
  UIButton *pDirectConn = reinterpret_cast<UIButton *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:DIRECTCONN"));
  UIButton *pHTTPConn = reinterpret_cast<UIButton *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:HTTPPROXY"));
  UIButton *pSOCKS4Conn = reinterpret_cast<UIButton *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SOCKS4PROXY"));
  UIButton *pSOCKS5Conn = reinterpret_cast<UIButton *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SOCKS5PROXY"));
  UIButton *pConnOK =
    reinterpret_cast<UIButton *>(m_GUI->getChild("EDITWEBCONF_FRAME:PROXYOK"));
  UIEdit *pServer = reinterpret_cast<UIEdit *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SUBFRAME:SERVEREDIT"));
  UIEdit *pPort = reinterpret_cast<UIEdit *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SUBFRAME:PORTEDIT"));
  UIEdit *pLogin = reinterpret_cast<UIEdit *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SUBFRAME:LOGINEDIT"));
  UIEdit *pPassword = reinterpret_cast<UIEdit *>(
    m_GUI->getChild("EDITWEBCONF_FRAME:SUBFRAME:PASSWORDEDIT"));

  if (pDirectConn->isClicked() || pHTTPConn->isClicked() ||
      pSOCKS4Conn->isClicked() || pSOCKS5Conn->isClicked()) {
  ...
  if (pConnOK->isClicked()) {
    ...
    XMSession::instance()->proxySettings()->setPort(
      atoi(pPort->getCaption().c_str()));
    XMSession::instance()->proxySettings()->setServer(pServer->getCaption());
    XMSession::instance()->proxySettings()->setAuthentification(
      pLogin->getCaption(), pPassword->getCaption());
...
void StateEditWebConfig::createGUIIfNeeded(RenderSurface *i_screen) {
  ...
  UIStatic *pWebConfEditorTitle =
    new UIStatic(v_frame, 0, 0, GAMETEXT_INETCONF, 400, 50);
...
#else
#define DIRCONNTEXT                           \
  (std::string(GAMETEXT_DIRECTCONN) + " / " + \
   std::string(GAMETEXT_USEENVVARS))          \
    .c_str()
#endif

  int radioButtonsGroup = 16023;

  UIButton *pConn1 = new UIButton(
    v_frame, 25, 60, DIRCONNTEXT, (v_frame->getPosition().nWidth - 50), 28);
  ...
  UIButton *pConn2 = new UIButton(v_frame,
                                  25,
                                  88,
                                  GAMETEXT_USINGHTTPPROXY,
                                  (v_frame->getPosition().nWidth - 160),
                                  28);
  ...
  UIButton *pConn3 = new UIButton(v_frame,
                                  25,
                                  116,
                                  GAMETEXT_USINGSOCKS4PROXY,
                                  (v_frame->getPosition().nWidth - 160),
                                  28);
  ...
  UIButton *pConn4 = new UIButton(v_frame,
                                  25,
                                  144,
                                  GAMETEXT_USINGSOCKS5PROXY,
                                  (v_frame->getPosition().nWidth - 160),
                                  28);
  ...
  UIButton *pConnOKButton =
    new UIButton(v_frame,
                 (v_frame->getPosition().nWidth - 160) + 28,
  ...
  UIFrame *pSubFrame = new UIFrame(v_frame,
                                   25,
                                   185,
                                   "",
                                   (v_frame->getPosition().nWidth - 50),
                                   (v_frame->getPosition().nHeight - 185 - 75));
  pSubFrame->setStyle(UI_FRAMESTYLE_TRANS);
  pSubFrame->setID("SUBFRAME");

  pSomeText = new UIStatic(
    pSubFrame, 10, 25, std::string(GAMETEXT_PROXYSERVER) + ":", 120, 25);
  ...
  pSomeText =
    new UIStatic(pSubFrame, 10, 55, std::string(GAMETEXT_PORT) + ":", 120, 25);
  pSomeText->setFont(drawLib->getFontSmall());
  pSomeText->setHAlign(UI_ALIGN_RIGHT);
  ...
  pSomeText =
    new UIStatic(pSubFrame, 10, 85, std::string(GAMETEXT_LOGIN) + ":", 120, 25);
  pSomeText->setFont(drawLib->getFontSmall());
  pSomeText->setHAlign(UI_ALIGN_RIGHT);
  ...
  pSomeText = new UIStatic(
    pSubFrame, 10, 115, std::string(GAMETEXT_PASSWORD) + ":", 120, 25);
  pSomeText->setFont(drawLib->getFontSmall());
  pSomeText->setHAlign(UI_ALIGN_RIGHT);
...
void StateEditWebConfig::updateGUI() {
  ...
  pLogin->setCaption(
    XMSession::instance()->proxySettings()->getAuthentificationUser());
  pPassword->setCaption(
    XMSession::instance()->proxySettings()->getAuthentificationPassword());
...
void StateEditWebConfig::sendFromMessageBox(const std::string &i_id,
