...
void DBuffer::writeBuf(_ConstIter pcBuf, int nBufSize) {
  if (isOutput() && nBufSize > 0) {
    ...
    while (nToWrite > 0) {
      ...
      if (m_Parts.empty())
        bNewPart = true;
      else if (m_Parts[m_nCurPart]->nPtr == m_nPartSize)
        bNewPart = true;
...
void DBuffer::readBuf(_Iter pcBuf, int nBufSize) {
  ...
  if (isInput() && nBufSize > 0) {
    ...
    if (m_nSize - m_nReadPtr < nBufSize) {
      ...
      std::copy(&m_pcData[m_nReadPtr], &m_pcData[m_nReadPtr] + nBufSize, pcBuf);
      m_nReadPtr += nBufSize;
    }
  }
}
...
void DBuffer::operator<<(unsigned long long n) {
  int sn;
  sn = (int)(n);
  *this << sn;
}
